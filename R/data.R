...
#'   \item{access_lat}{Numeric. Latitude of the shoreline access point (missing for most lakes).}
#'   \item{access_long}{Numeric. Longitude of the shoreline access point (missing for most lakes).}
#'   \item{lake_lat}{Numeric. Lake latitude (outlet or centroid).}
#'   \item{lake_long}{Numeric. Lake longitude (outlet or centroid).}
...
# ----------------------------------------------------------------------
#' Lake elevation and ice phenology estimates
#'
#' One row per lake (WBID) giving a best-available surface elevation and the
#' estimated typical ice-on / ice-off timing. Built once (e.g. annually) so the
#' slow remote elevation lookup in \code{find_elevation()} does not have to run
#' on every \code{Lakes} build; \code{Create_Lakes.R} joins this table in and
#' fills any lakes that are missing a DataPond elevation. All values are
#' estimates/derived, not direct observations.
#'
#' @format A data frame with one row per waterbody and the following columns:
#' \describe{
#'   \item{WBID}{Character. Unique waterbody identifier (leading zeros preserved).}
#'   \item{elevation_est}{Numeric. Best-available surface elevation in metres: the
#'     DataPond \code{elevation_m} where present, otherwise the estimate from
#'     \code{find_elevation()}. Drives the ice estimate below.}
#'   \item{ice_on_day}{Integer. Estimated typical ice-on day of year, or \code{NA}
#'     for lakes that do not typically freeze (from \code{estimate_lake_ice()}).}
#'   \item{ice_off_day}{Integer. Estimated typical ice-off day of year (from
#'     \code{estimate_lake_ice()}).}
#' }
#'
#' @usage data(Elev_ice_estimates)
#'
#' @source Built by \code{data-raw/Create_Elev_ice_estimates.R} from DataPond waterbody
#'   views, with elevations from \code{find_elevation()} (elevatr) and ice timing
#'   from \code{estimate_lake_ice()}.
#'
#' @examples
#' data(Elev_ice_estimates)
#' head(Elev_ice_estimates)
"Elev_ice_estimates"
NULL

# ----------------------------------------------------------------------
#' Known FISS fish observations by lake
#'
#' A raw, per-observation table of known fish drawn from the provincial FISS
#' "Known BC Fish Observations" point layer (POINT_TYPE_CODE == "Observation"
#' only), restricted to lakes and species DP2R knows about. Kept unsummarised so
#' it can be summarised different ways later; \code{linkClips()} derives its own
#' first/last observation year from it for \code{Lake_Spp}. Built once (e.g.
#' annually) rather than re-querying the BC Data Catalogue.
#'
#' @format A data frame with one row per distinct observation and the following columns:
#' \describe{
#'   \item{WBID}{Character. Waterbody identifier, matching the DataPond WBID
#'     (the provincial waterbody_identifier).}
#'   \item{species_code}{Character. FISS species code as published in the catalogue.}
#'   \item{year}{Integer. Calendar year extracted from OBSERVATION_DATE.}
#'   \item{activity}{Character. Activity / sampling method of the observation
#'     (FISS ACTIVITY field).}
#'   \item{agency_name}{Character. Agency that reported the observation.}
#' }
#'
#' @usage data(FISS_Spp)
#'
#' @source Generated by \code{data-raw/Create_FISS_Spp.R} from the bcdata R
#'   package (BC Data Catalogue: Known BC Fish Observations and BC Fish
#'   Distributions).
#'
#' @examples
#' data(FISS_Spp)
