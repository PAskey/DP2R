...
EffortClean <- function() {
  ...
  #FREEZING LAKES
  #Should_freeze = vwWaterbodyLake%>%filter(!region %in% c("1","2"), area_ha<1900, elevation_m >300|is.na(elevation_m))%>%pull(WBID)
...
#Step 3. Remove all duplicates (About 15000).
  ...
  Edata_dt <- Edata_dt[
    order(WBID, method, assessed_dt, num_shore_ice, num_spv, num_boat, num_ice_tent, view_location_name, is.na(comment)),
    .SD[1],  # Keep the first occurrence (non-NA comment prioritized = ~150 more comments)
  ...
  removals <- unique(c(remove_NAs$fishing_effort_id, remove_night$fishing_effort_id, remove_vis$fishing_effort_id))
